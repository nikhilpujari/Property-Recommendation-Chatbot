...
const Chatbot = () => {
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Creating new chat user with:');
      console.log('- API URL:', `${API_BASE_URL}/api/chat/users`);
      console.log('- Name:', name);
      console.log('- Contact:', contact);
      
      // Save user to the database - using axios to ensure correct URL formatting
      const response = await axios.post(`${API_BASE_URL}/api/chat/users`, {
        name,
        contact,
        conversation
      });
      
      console.log('Successfully created user. Status:', response.status);
      
      setUserId(response.data.id);
      console.log('Saved user ID:', response.data.id);
...
Monthly Payment: ${formatCurrency(results.monthlyPayment)}
  ...
  const handleMessageSubmit = async (e: React.FormEvent) => {
    ...
    if (step === 'greeting') {
      handleGreeting(currentMessage);
    } else if (step === 'conversation') {
      // Default conversation flow
      setTimeout(() => {
        const botResponse: ConversationMessage = {
          role: 'bot',
          message: "Thank you for the information. One of our real estate experts will follow up with more details soon. Is there anything else you'd like to know?",
          timestamp: Date.now()
        };
        ...
        // Update the conversation in the database with latest messages
        updateConversationInDb([...updatedConversation, botResponse]);
      }, 1000);
    }
    
    // Update the conversation in the database with user message
    if (userId) {
      updateConversationInDb(updatedConversation);
    }
  }
  
  // Helper function to update conversation in the database
  const updateConversationInDb = async (conversationData: ConversationMessage[]) => {
    if (!userId) {
      console.log('Cannot update conversation: userId is null');
      return;
    }
    
    try {
      // Ensure the URL is correctly formatted with forward slash
      // Make sure userId is properly converted to a string if it's a number
      const chatUserId = String(userId);
      const url = `/api/chat/users/${chatUserId}`;
      
      // Log details for debugging
      console.log('Making PATCH request to update conversation:');
      console.log('- Full URL that will be used:', `${API_BASE_URL}${url}`);
      console.log('- userId:', chatUserId, 'type:', typeof chatUserId);
      console.log('- conversation length:', conversationData.length);
      
      // Use axios directly with full URL to ensure correct formatting
      const axiosResponse = await axios.patch(`${API_BASE_URL}${url}`, { 
        conversation: conversationData 
      });
      
      console.log('Successfully updated conversation. Status:', axiosResponse.status);
    } catch (error: any) {
  ...
  // Log customer lead to database
  const logCustomerLead = async (interest: string, propertyDetails?: any) => {
    try {
      if (!name || !contact) {
        console.log('Cannot log lead: missing name or contact information');
        return false;
      }
      ...
      const leadData = {
        ...
        notes: notes,
        conversation: conversation // Pass the current conversation
      };
      
      // Only log significant actions or if this is the first interaction
      ...
      if (!leadLogged || isSignificantAction) {
        console.log('Logging lead to database:', interest);
        console.log('API URL:', `${API_BASE_URL}/api/sheets/log-lead`);
        
        // We're still using the sheets/log-lead endpoint which now creates/updates leads in the database
        const response = await axios.post(`${API_BASE_URL}/api/sheets/log-lead`, leadData);
        
        console.log('Lead logging response status:', response.status);
        
        if (response.status === 201 || response.status === 200) {
          setLeadLogged(true);
          console.log('Successfully logged lead to database');
          
          // If this is a new chat user (no userId yet), update the conversation in the database
          if (!userId && response.data && response.data.leadId) {
            setUserId(response.data.leadId);
            console.log('Updated userId from lead response:', response.data.leadId);
            
            // After setting the userId, update the conversation in the database
            setTimeout(() => {
              updateConversationInDb(conversation);
            }, 100);
          } else if (userId) {
            // Update conversation in the database if we already have a userId
            updateConversationInDb(conversation);
          }
        ...
        console.log('Skipping lead log - not first interaction or significant action');
        
        // Still update the conversation in the database
        if (userId) {
          updateConversationInDb(conversation);
        }
      }
      ...
      console.error('API URL attempted:', `${API_BASE_URL}/api/sheets/log-lead`);
